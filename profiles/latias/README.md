...
This README is specific to the `latias` profile.
...
```bash
sudo nixos-rebuild boot --flake ~/.dotfiles/nixos#latias
```
...
sudo ./secrets-manager import . --profile latias
```
...
```bash
sudo nix run github:Fran314/secrets-manager-rs -- import /path/to/usb/mountpoint  --profile latias
```
